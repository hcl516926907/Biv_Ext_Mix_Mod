run_MCMC_parallel <- function(seed, dat, niter, nburnin, thin){
  ...
  R_pmnorm <- function(x, mean, sigma, rho){
    norm.cop <- ellipCopula("normal", param = rho, dim = 2)
    gaussian <- mvdc(norm.cop, margins = c('norm','norm'), 
                     paramMargins=list(list(mean=mean[1],sd=sigma[1]),
                                       list(mean=mean[2],sd=sigma[2])))
    p <- pMvdc(x, gaussian)
    return(p)
  }
  
  
  R_dmvnorm <- function(x, mean, sigma, rho, log){
    norm.cop <- ellipCopula("normal", param = rho, dim = 2)
    gaussian <- mvdc(norm.cop, margins = c('norm','norm'), 
                     paramMargins=list(list(mean=mean[1],sd=sigma[1]),
                                       list(mean=mean[2],sd=sigma[2])))
  ...
  nim_pmnorm<- nimbleRcall(function(x = double(1), mean=double(1), sigma=double(1),
                                 rho = double(0)){}, 
                             Rfun = 'R_pmnorm',
                             returnType = double(0))
  ...
  nim_dmvnorm <- nimbleRcall(function(x = double(2), mean = double(1), 
                                       sigma=double(1), rho = double(0),
  ...
  dbiextmix <- nimbleFunction(
    run = function(x=double(2), theta=double(1), thres=double(1), mu=double(1), 
                   sigma=double(1), rho=double(0), D=integer(0, default=2),
      ...
      pi <- nim_pmnorm(thres, mean=mu, sigma = sigma, rho=rho)
      ...
      if (n.bulk>0){
        y.bulk <- x[!cond,]
        dbulk <- nim_dmvnorm(y.bulk, mean=mu, sigma = sigma, rho=rho, log = log )
      }
      if (log) {
  ...
  # theta=c(a,exp(beta)[1],sig,gamma)
  # thres=u.x
  # mu=mu
  # sigma=c(sd1,sd2)
  # rho=rho
  ...
  # gamma.ind=c(6,7)
  # log=TRUE
  # dbiextmix(matrix(c(1,2,3,4),ncol=2), theta=c(a,exp(beta)[1],sig,gamma),thres=u.x,mu=mu,sigma=c(sd1,sd2),rho=rho, a.ind=c(1,2),lam.ind=3,sig.ind=c(4,5),gamma.ind=c(6,7),log=TRUE)
  ...
  registerDistributions(list(
    dbiextmix = list(
      BUGSdist = "dbiextmix(theta, thres, mu, sigma,rho, D, a.ind, lam.ind, lamfix, sig.ind, gamma.ind)",
      types = c('value = double(2)', 'theta = double(1)', 'thres = double(1)', 
                'mu = double(1)', 'sigma = double(1)', 'rho=double(0)', 'D = integer(0)', 'a.ind = double(1)', 
  ...
  assign('R_pmnorm', R_pmnorm, envir = .GlobalEnv)
  assign('R_dmvnorm', R_dmvnorm, envir = .GlobalEnv)
  assign('nim_pmnorm', nim_pmnorm, envir = .GlobalEnv)
  assign('nim_dmvnorm', nim_dmvnorm, envir = .GlobalEnv)
