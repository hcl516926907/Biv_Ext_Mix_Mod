...
#------------------------------tail proportaiton 5%, 10 times larger size---------------------
...
# Simulate with (conditionally) GP(sig,gamma) and (conditionally) exponential margins 
X.tail<-sim.RevExpU.MGPD(n=1250,d=d, a=a, beta=beta, sig=sig, gamma=gamma, MGPD = T,std=T)
...
# GP scale tail data combined with the bulk data
set.seed(1234)
X.bulk <- rtmvnorm(23750, mean=u.x-c(2,1.8), sigma=1.5*sigma, lower=c(0,0),upper=u.x)

X <- rbind(X.bulk, sweep(X.tail$X,2,u.x,"+"))
plot(X,pch=20)
plot(X,pch=20, xlab='Z1', ylab='Z2', main='Simulated tNormal + GP Data',
     col=c(rep('grey50',dim(X.bulk)[1]),rep('black',dim(X.tail$X)[1])))
segments(u.x[1], 0, u.x[1], u.x[2], col='red', lty=2)
segments(0, u.x[2], u.x[1], u.x[2], col='red', lty=2)
legend('bottomright',   legend = c("Tail Data", "Bulk Data", "Threshold"), lty=c(NA,NA,2),col = c('black', 'grey50', 'red'),         
       pch=c(20,20,NA)) 
...
X.p05.10t <- X
u.x.p05.10t <- u.x
save(X.p05.10t, X.p05.10t,
     file=file.path(dir.out,'simulation_data_pos_shape_10t.RData'))
